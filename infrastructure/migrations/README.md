...
M2 的 `0004`–`0006` 增加核心实体、组合租户外键、版本触发器、Project OWNER 写入策略、与 Project 写入同事务的 Audit/Outbox，以及 Project 创建幂等记录。Task 等其他实体目前仅有 schema 和只读 RLS，repository 与生命周期仍在实施。`pnpm test:core` 运行数据库不变量与原子性测试；这些迁移只在开发环境验证，不能直接作为生产部署方案。
