...
桌面有 Home、Chat、Work、Build、Projects、Activity、Settings，支持页面搜索、上下文收起、分类切换，以及 Chat/Work 草稿在此设备自动保存、从首页继续和复制/导出。Chat 预览可把想法保存为本机笔记，并提供导出与删除。个人工作区可在此设备创建 Work/Build 项目、添加并完成本机任务，从首页继续指定项目；演示工作区的示例内容明确标记，且不混入个人数据。这些本机内容尚未同步到账号，也不会发送给 AI。服务端 Conversation 消息存储已实现，但 Desktop 尚未接入身份服务或该 API。账户、模型和正式任务服务未连接，不伪造执行结果。设计与验收记录见 `design-system/`、`design-qa.md` 和 `docs/implementation/UI.md`。
